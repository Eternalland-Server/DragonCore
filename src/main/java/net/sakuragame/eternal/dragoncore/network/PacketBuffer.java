...
public class PacketBuffer extends ByteBuf {
    ...
    private static Constructor<?> constructor_PacketDataSerializer;
    private static Method method_writeItemStack;
    private static Method method_toNMSItemStack;

    static {
        String packet = Bukkit.getServer().getClass().getPackage().getName();
        String nmsName = "net.minecraft.server." + packet.substring(packet.lastIndexOf(46) + 1);
        try {
            Class<?> class_ItemStack = Class.forName(nmsName + ".ItemStack");
            constructor_PacketDataSerializer = Class.forName(nmsName + ".PacketDataSerializer").getConstructor(ByteBuf.class);
            method_writeItemStack = Class.forName(nmsName + ".PacketDataSerializer").getDeclaredMethod("a", class_ItemStack);
            method_toNMSItemStack = Class.forName(packet + ".inventory.CraftItemStack").getDeclaredMethod("asNMSCopy", ItemStack.class);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void writeItemStack(ItemStack itemStack) {
        try {
            Object packetDataSerializer = constructor_PacketDataSerializer.newInstance(this);
            Object nmsItemStack = method_toNMSItemStack.invoke(null, itemStack);
            method_writeItemStack.invoke(packetDataSerializer, nmsItemStack);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
